    ...
    class IndexAsTable < ActiveAdmin::Component
        ...
        def id_column
          column "id" do |row|
            ...
            if controller.action_methods.include?("show")
              link_to id, "/#{DirectiveAdmin.namespace.name}/#{@resource_class.name.tableize}/#{id}"
            else
              id
            end
          end
      ...
      private
        ...
        def defaults(row, options = {})
          path = "/#{DirectiveAdmin.namespace.name}/#{@resource_class.name.demodulize.tableize}/#{row["id"]}"
